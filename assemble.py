...
def copy_container_images_to_wic(target: FactoryClient.Target, app_image_dir: str,
    ...
    if not target_app_store.exist(target):
        ...
        apps_fetcher = TargetAppsFetcher(token, app_image_dir)
...
if __name__ == '__main__':
    ...
    try:
        ...
        for target in targets:
            logger.info('Assembling image for {}'.format(target.name))
            image_file_path = factory_client.get_target_system_image(target, args.out_image_dir)
            copy_container_images_to_wic(target, args.app_image_dir, image_file_path, args.token, args.app_shortlist)
